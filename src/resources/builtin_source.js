...
const searchKuwoMatch = async (title, artist, quality) => {
  ...
  const searchKey = cleanTitle + (cleanFirstArtist ? (' ' + cleanFirstArtist) : '');
  const searchUrl = 'https://search.kuwo.cn/r.s?client=kt&all=' + encodeURIComponent(searchKey) + '&pn=0&rn=5&ver=kwplayer_ar_9.2.2.1&vipver=1&show_copyright_off=1&newsearch=1&ft=music&cluster=0&strategy=2012&encoding=utf8&rformat=json&vermerge=1&mobi=1';
...
const handleMusicUrl = async (songInfo, type, source) => {
  ...
  try {
    ...
    // 通道 B: 跨平台曲目客户端国内毫秒级原唱强匹配（400ms 出 FLAC）
